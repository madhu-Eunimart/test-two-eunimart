...
function SearchRequest(data) {
  ...
  const searchRequest = {
    ...
    message: {
      intent: {
        ...
        ...((data?.provider_id || data?.category_id || data?.provider_name) && {
          provider: {
            ...(data?.provider_id && {
              id: data?.provider_id
            }),
  ...
  if (data?.area_code) {
    searchRequest.message.intent.fulfillment.end.location["address"] = {
...
export { SearchRequest, SelectRequest, InitRequest, UpdateRequest, ConfirmRequest, TrackRequest, SupportRequest, CancelRequest, StatusRequest };