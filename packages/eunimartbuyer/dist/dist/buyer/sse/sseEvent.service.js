...
class SseEvent extends EventEmitter {
  constructor(initial, options = {}) {
    super();
    if (initial) this.initial = initial;else this.initial = [];
    if (!_.isEmpty(options)) this.options = {
      ...options
    };else this.options = {
      isSerialized: true
    };
  ...
  init(req, res, time) {
    ...
    if (this.initial) {
      if (this.options?.isSerialized) this.serialize(this.initial);else if (!_.isEmpty(this.initial)) this.send(this.initial, this.options.initialEvent || false, this.options.eventId);
    }
