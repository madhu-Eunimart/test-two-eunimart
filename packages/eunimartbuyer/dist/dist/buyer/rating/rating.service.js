...
class RatingService {
  ...
  async BapRating(orderRequest) {
    try {
      ...
      const bppResponse = await bppRatingService.BapRating(requestContext.bpp_uri, context, message);
      return bppResponse;
    } catch (err) {
      throw err;
    }
  ...
  async ONDCRatingOrder(orderRequest, isMultiSellerRequest = false) {
    try {
      ...
      const bppResponse = await bppRatingService.ONDCRating(requestContext.bpp_uri, orderRequest);
      return bppResponse;
    } catch (err) {
      throw err;
    }
  ...
  async ONDCRatingCategoriesEvent(orderRequest, isMultiSellerRequest = false) {
    try {
      ...
      //       }
      // console.log("inside client")
      const bppResponse = await bppRatingService.ONDCRatingCategories(requestContext.bpp_uri, orderRequest);
      return bppResponse;
    } catch (err) {
      throw err;
    }
  ...
  async ONDCFeedbackCategoriesEvent(orderRequest, isMultiSellerRequest = false) {
    try {
      ...
      const bppResponse = await bppRatingService.ONDCFeedbackCategories(requestContext.bpp_uri, orderRequest);
      return bppResponse;
    } catch (err) {
      throw err;
    }
  ...
  async ONDCFeedbackFormEvent(orderRequest, isMultiSellerRequest = false) {
    try {
      ...
      const bppResponse = await bppRatingService.ONDCFeedbackForm(requestContext.bpp_uri, orderRequest);
      return bppResponse;
    } catch (err) {
      throw err;
    }
  ...
  async satingMultipleOrder(requests) {
    const satingResponses = await Promise.all(requests.map(async request => {
      try {
  ...
  async onRating(messageId) {
    try {
      ...
      if (protocolRatingResponse && protocolRatingResponse.length) return protocolRatingResponse?.[0];else {
        const contextFactory = new ContextFactory();
        const context = contextFactory.create({
          messageId: messageId,
  ...
  async onRatingMultipleOrder(messageIds) {
    try {
      const onRatingResponse = await Promise.all(messageIds.map(async messageId => {
        try {
  ...
  async bppOnRatingResponse(uri, orderRequest) {
    try {
      ...
      const bppResponse = await bppRatingService.bppOnRatingResponse(uri, context, message);
      return bppResponse;
    } catch (err) {
      throw err;
    }
  ...
  */
  async getBapRatingCategories(orderRequest) {
    try {
      ...
      const bppCategoriesResponse = await bppRatingService.getBppRatingCategoriesService(requestContext.bpp_uri, context);
  ...
  async getBapFeedbackCategories(orderRequest) {
    try {
      ...
      const bppCategoriesResponse = await bppRatingService.getBppFeedbackCategoriesService(requestContext.bpp_uri, context);
      return bppCategoriesResponse;
    } catch (err) {
      throw err;
    }
  ...
  async getBapFeedbackForm(orderRequest) {
    try {
      ...
      const bppCategoriesResponse = await bppRatingService.getBppFeedbackFormService(requestContext.bpp_uri, context, message);
      return bppCategoriesResponse;
    } catch (err) {
      throw err;
    }
