...
class RatingController {
  ...
  */
  onRating(req, res, next) {
    ...
    if (messageId && messageId.length) ratingService.onRating(messageId).then(response => {
      res.json(response);
    }).catch(err => {
      next(err);
  ...
  */
  getBapFeedbackCategories(req, res, next) {
    const {
      body: feedbackCategoriesRequest
    } = req;
  ...
  */
  getBppRatingCategories(req, res, next) {
    ...
    isSignatureValid(proxy_auth, req.body, SUBSCRIBER_TYPE.BAP).then(async isValid => {
      if (!isValid) {
        return res.status(401).setHeader('Proxy-Authenticate', proxy_auth).json({
          message: {
            "ack": {
              "status": "NACK"
            },
