...
class ReturnOrderService {
  ...
  */
  async returnOrder(orderRequest) {
    try {
      ...
      return await bppReturnService.returnOrder(context, order_id, return_reason_id);
    } catch (err) {
      throw err;
    }
  }
  ...
  */
  async ONDCReturnOrder(orderRequest) {
    try {
      ...
      return await bppReturnService.ONDCReturnOrder(requestContext.bpp_uri, requestContext, orderRequest);
    } catch (err) {
      throw err;
    }
  }
    ...
    try {
      ...
      if (!(protocolReturnResponse && protocolReturnResponse.length)) {
        ...
        if (!protocolReturnResponse?.[0].error) {
          ...
          if (!(dbResponse || dbResponse.length)) throw new NoRecordFoundError();else {
  ...
  */
  async bppOnReturnOrderResponse(uri, orderRequest) {
    try {
      const bppResponse = await bppReturnService.bppOnReturnResponse(uri, orderRequest);
      return bppResponse;
    } catch (err) {
      throw err;
    }
