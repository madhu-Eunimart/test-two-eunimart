  ...
  async selectOrder(orderRequest) {
    try {
      ...
      const {
        order = {},
        fulfillments = [],
        CreatedBy = orderRequest?.message?.CreatedBy
      } = message;
      ...
      return await bppSelectService.select(requestContext.bpp_uri, context, {
        order,
        fulfillments,
        CreatedBy
      });
  ...
  */
  async ONDCSelectOrder(orderRequest) {
    try {
      ...
      return await bppSelectService.ONDCSelect(requestContext.bpp_uri, orderRequest);
    } catch (err) {
      throw err;
    }
  }
