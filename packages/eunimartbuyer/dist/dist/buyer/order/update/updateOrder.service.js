  ...
  */
  async updateOrder(orderRequest, isMultiSellerRequest = false) {
    try {
      ...
      const bppResponse = await bppUpdateService.update(requestContext.bpp_uri, context, "item", message?.order);
      return bppResponse;
    } catch (err) {
      throw err;
    }
  ...
  */
  async ONDCUpdateOrder(orderRequest, isMultiSellerRequest = false) {
    try {
      ...
      const bppResponse = await bppUpdateService.ONDCUpdate(requestContext.bpp_uri, orderRequest);
      return bppResponse;
    } catch (err) {
      throw err;
    }
