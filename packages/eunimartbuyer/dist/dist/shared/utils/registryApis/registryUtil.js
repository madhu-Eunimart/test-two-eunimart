...
export const getSubscriberUrl = (subscriberDetails = []) => {
  try {
    ...
    if (process.env.ENV_TYPE !== "STAGING" && subscriber_id && subscriber_url || process.env.ENV_TYPE === "STAGING" && subscriber?.subscriber_url) return process.env.ENV_TYPE !== "STAGING" ? `https://${subscriber_id}${subscriber_url}` : subscriber?.subscriber_url;else throw new Error("Invalid subscriber url");
  } catch (err) {
    throw err;
  }
};
