...
const lookupBppById = async ({
  ...
  Configuration?.REGISTRY_BASE_URL, registryBaseUrl, "POST", {
    ...request
  });
...
const lookupRsp = async request => {
  let registryBaseUrl = REGISTRY_SERVICE_API_URLS.LOOKUP;
  const apiCall = new HttpRequest(Configuration?.REGISTRY_BASE_URL, registryBaseUrl, "POST", request);
  const result = await apiCall.send();
  return result.data;
};
...
const lookupGateways = async (subscriber_id = Configuration?.BG_ID) => {
  ...
  const apiCall = new HttpRequest(Configuration?.REGISTRY_BASE_URL, registryBaseUrl, "POST", {
    ...request
  });
  const result = await apiCall.send();
