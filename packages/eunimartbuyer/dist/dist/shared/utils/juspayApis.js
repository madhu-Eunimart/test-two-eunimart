...
const getJuspayOrderStatus = async orderId => {
  let apiCall = new FetchRequest(process.env.JUSPAY_BASE_URL, "/orders/" + orderId, "GET", {}, {
  ...
  await apiCall.send().then(response => {
    if (response.status === "error" || response.error) throw response;else paymentDetails = response;
  }).catch(err => {
    throw err;
  });
