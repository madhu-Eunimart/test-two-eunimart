...
const cacheSearch = async searchRequest => {
  ...
  let query = {
    ...(provider_string && {
      "descriptor.name": new RegExp(provider_string, "i")
    }),
  ...
  for (let provider of serviceableProviders) {
    ...
    const query = {
      ...(search_string && {
        "descriptor.name": new RegExp(search_string, "i")
      }),
      ...(category_string && {
        category_id: new RegExp(category_string, "i")
      }),
      ...(provider.locations && {
        location_id: {
          "$in": locs
    ...
    console.log(query);
    let items = await BapSearchItemCache.find(query, {
      _id: 0,
      __v: 0,
      provider_id: 0,
      bpp_id: 0
          ...
          for (let j = 0; j < tag_array.length; j++) {
            if (tag_array[j]['location'] == location.id || tag_array[j]['location'] == '*') {
              if (tag_array[j].type == "12" && tag_array[j].unit == 'country' && tag_array[j].val == 'IND') {
                ...
                if (!obj) {
                  serviceableLocations.push(location);
                }
